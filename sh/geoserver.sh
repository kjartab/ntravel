...
#	Install Java7 x64
#
#wget --no-check-certificate --quiet  --no-cookies - --header "Cookie: oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/7u79-b01/jre-7u79-linux-x64.tar.gz
...
sudo mv jdk-7u79-linux-x64.tar.gz /usr/lib/jvm
...
echo -e "JAVA_HOME=/usr/lib/jvm/jdk1.7.0_79" | sudo tee -a /etc/profile
...
sudo wget  http://sourceforge.net/projects/geoserver/files/GeoServer/2.7.1/geoserver-2.7.1-war.zip
